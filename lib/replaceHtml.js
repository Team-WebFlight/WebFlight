...
//////////////////////
  ...
  let hey = htmlString.replace(/<%([\w\W]*?)%>/g, function(match, subMatch) {return "<!-- <%"+subMatch+"%> -->" });

    console.log('htmlString💆💆💆💆💆',htmlString.replace(/<%([\w\W]*?)%>/g, "<!-- <%([\w\W]*?)%> -->"), 'HEYYYYYYYYYYYYYY', hey);
    const $ = cheerio.load(hey)
